package com.example.mygrub.admin.settings;
...
public class ChangePassA extends AppCompatActivity {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    ...
                    try {
                        ...
                        Toast.makeText(ChangePassA.this, e.getMessage(), Toast.LENGTH_LONG).show();
                    }
                    loading.setVisibility(View.GONE);
                }
            }
    ...
    private void updatePassword(String newPass) {
        user.updatePassword(newPass).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    Toast.makeText(ChangePassA.this, "Password changed successfully!", Toast.LENGTH_SHORT).show();
                    onBackPressed();
                } else {
                    try {
                        throw task.getException();
